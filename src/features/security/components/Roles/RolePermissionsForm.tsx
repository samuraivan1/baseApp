...
export default function RolePermissionsForm({ role, onClose }: Props) {
  ...
  const AvailZone: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return <div ref={setNodeRef} className={`${styles['rp-drop']} ${isOver ? styles['rp-list--over'] : ''}`}>{children}</div>;
  };
  const AssignZone: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const pagedAvailable = available.slice((pageAvail - 1) * pageSize, pageAvail * pageSize);
  const pagedSelected = selected.slice((pageAssign - 1) * pageSize, pageAssign * pageSize);

  useEffect(() => { setPageAvail(1); }, [available.length]);
  useEffect(() => { setPageAssign(1); }, [selected.length]);

  // dnd-kit sensors (mouse, touch, keyboard)
  const sensors = useSensors(
    useSensor(MouseSensor, { activationConstraint: { distance: 5 } }),
    useSensor(TouchSensor, { pressDelay: 120, activationConstraint: { distance: 3 } }),
  ...
  return (
    <div className={styles.rolePerms}>
      <SectionHeader title={`Permisos para: ${role.name}`} icon={faKey} onBack={onClose} />
      <ListLoading loading={loading || permsQuery.isLoading} message="Cargando permisos..." showSpinner spinnerSize="lg" layout="centered">
        <div className={styles.rolePerms__columns}>
          <div className={styles.rolePerms__col}>
            ...
            <div className={styles['rp-pagination']}>
              <button className={styles['rp-pagination__btn']} onClick={() => setPageAvail((p) => Math.max(1, p - 1))} disabled={pageAvail === 1}>Anterior</button>
              <span>{pageAvail} / {totalAvailPages}</span>
              <button className={styles['rp-pagination__btn']} onClick={() => setPageAvail((p) => Math.min(totalAvailPages, p + 1))} disabled={pageAvail === totalAvailPages}>Siguiente</button>
            </div>
          </div>
          <div className={styles.rolePerms__col}>
            <div className={styles.rolePerms__titlebar}>
              <h4 className={styles.rolePerms__title}>Asignados ({selected.length})</h4>
              {!!assigned.length && (
                <button type="button" className={styles['rp-pagination__btn']} style={{ background: '#fdecec', borderColor: '#dc2626', color: '#991b1b' }} onClick={handleClear}>Quitar todos</button>
              )}
            </div>
            <DndContext sensors={sensors} onDragEnd={onDragEnd}>
                <AssignZone>
            ...
            <div className={styles['rp-pagination']}>
              <button className={styles['rp-pagination__btn']} onClick={() => setPageAssign((p) => Math.max(1, p - 1))} disabled={pageAssign === 1}>Anterior</button>
              <span>{pageAssign} / {totalAssignPages}</span>
              <button className={styles['rp-pagination__btn']} onClick={() => setPageAssign((p) => Math.min(totalAssignPages, p + 1))} disabled={pageAssign === totalAssignPages}>Siguiente</button>
            </div>
          </div>
        </div>
