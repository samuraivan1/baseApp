...
export function useProductsCrud() {
  const crud = useEntityCrud<Product, ProductInput>('products', {
    list: getProducts,
  ...
  const create = useSafeMutation<Product, ProductInput>(createProduct, {
    onSuccess: () => qc.invalidateQueries({ queryKey: crud.keys.all }),
  });
  const update = useSafeMutation<Product, { id: number; input: ProductInput }>(
